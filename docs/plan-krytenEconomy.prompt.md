...
8. **Zero upstream risk** — no changes to CyTube, Kryten-Robot, or kryten-py. Pure microservice. All interaction via NATS events and commands.
...
│   ├── command_handler.py         # NATS request-reply on kryten.economy.command
...
|---|---|---|
| `kryten.economy.command` | Request-reply | API for other services and admin tools |
| `cytube.commands.{channel}.chat` | Publish | Public channel announcements |
| `cytube.commands.{channel}.pm` | Publish | PM responses to users |
| `cytube.commands.{channel}.addvideo` | Publish | Queue content from MediaCMS |
| `kryten.lifecycle.economy.*` | Publish | Startup/shutdown/heartbeat (auto via KrytenClient) |

### Service Startup Sequence

Follows the canonical kryten-userstats pattern:
...
7. Subscribe to `kryten.lifecycle.robot.startup` for re-initialization
...
6. **PM handler** (`pm_handler.py`): Subscribe to `pm` events. Parse PM text as commands. Implement `balance` (returns balance, rank name, streak). Implement `help` (brief, playful, non-exhaustive).
7. **Command handler** (`command_handler.py`): NATS request-reply on `kryten.economy.command`. Implement `system.ping`, `system.health`, `balance.get`. Follow the kryten-userstats `StatsPublisher` pattern with handler routing map.
...
1. **Gambling engine** (`gambling_engine.py`): Manage all game types. Validate wagers. Record outcomes in `transactions` and `gambling_stats`.
...
3. **Queue commands**: `search`, `queue <id>`, `playnext <id>`, `forcenow <id>`. Publish `cytube.commands.{channel}.addvideo` via NATS.
...
7. **CyTube level 2 promotion**: Via achievement auto-promote or purchase. Publish CyTube rank-change command via NATS.
...
1. **Event announcer** (`event_announcer.py`): Centralized announcement engine. Configurable templates. Publish to public chat via NATS.
...
Sprint 1 (Foundation)
   ...
   │         │
   │         └──→ Sprint 7 (Events & Bounties) ──→ Sprint 8 (Admin & Reporting)
   │                                                       │
...
4. **PM event format** — Verify how kryten-robot publishes PMs to NATS. Expected: `pm` event arriving as `ChatMessageEvent`. Confirm channel/domain fields.
...
7. **CyTube rank change command** — Verify NATS command format to change a user's CyTube level (for level 2 promotion).
